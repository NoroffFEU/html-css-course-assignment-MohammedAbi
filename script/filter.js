...
function createFilterElement(label, value, products) {
    ...
    radioInput.addEventListener("change", (event) => filterProducts(products, event));
...
function renderGenderFilter(products) {
    try {
        ...
        genderLabels.forEach((label, index) => {
            const [radioInput, radioLabel] = createFilterElement(label, label, products);
        ...
        productContainer.parentNode.insertBefore(newFilterContainer, productContainer);
    } catch (error) {
        console.error("Error rendering gender filter:", error);
    }
}
...
function filterProducts(products, event) {
    const selectedGender = event.target.value;
    const filteredProducts = selectedGender === "all" ? products : products.filter((product) => product.gender.toLowerCase() === selectedGender);
    renderFilteredProducts(filteredProducts);
}
